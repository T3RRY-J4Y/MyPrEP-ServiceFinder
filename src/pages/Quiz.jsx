    ...
    <div style={{ background: bg, minHeight: "100vh", paddingBottom: 100 }}>
      ...
      <div style={{
        ...
        <div style={{ position: "relative", zIndex: 1 }}>
          ...
          <h1 style={{ color: "#fff", fontWeight: 700, fontSize: "clamp(2rem, 4.5vw, 2.625rem)", marginBottom: 8 }}>
            {d.label}
          </h1>
          {/* Effectiveness pill */}
          <span style={{
      ...
      <div style={{ maxWidth: 760, margin: "0 auto", padding: "28px 20px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
          <div style={{
            ...
            <p style={{ color: d.color, fontWeight: 700, fontSize: "0.82rem", textTransform: "uppercase", letterSpacing: ".06em", marginBottom: 10 }}>
              💡 What is it?
            </p>
            <p style={{ color: "#333", fontSize: "0.938rem", lineHeight: 1.65 }}>{d.what}</p>
          </div>
          <div style={{
            background: "#fff", borderRadius: 20, padding: "22px",
            borderTop: "4px solid " + d.color,
            ...
            <p style={{ color: "#333", fontSize: "0.938rem", lineHeight: 1.65 }}>{d.how}</p>
          </div>
        </div>
        ...
        <div style={{
          ...
          <p style={{ color: "rgba(255,255,255,0.8)", fontWeight: 700, fontSize: "0.82rem", textTransform: "uppercase", letterSpacing: ".06em", marginBottom: 10 }}>
            📋 How is it used?
          </p>
          <p style={{ color: "#fff", fontSize: "0.938rem", lineHeight: 1.7 }}>{d.used}</p>
        </div>
        ...
        <div style={{
          ...
          <p style={{ color: d.color, fontWeight: 700, fontSize: "0.82rem", textTransform: "uppercase", letterSpacing: ".06em", marginBottom: 16 }}>
            🗓 The process
          </p>
            ...
            {d.process.map((p, i) => (
              <div key={i} style={{ display: "flex", gap: 20, marginBottom: i < d.process.length - 1 ? 20 : 0, position: "relative" }}>
                ...
                <div style={{ paddingTop: 8 }}>
                  <p style={{ color: d.color, fontWeight: 700, fontSize: "0.875rem", marginBottom: 2 }}>{p.time}</p>
                  <p style={{ color: "#444", fontSize: "0.938rem", lineHeight: 1.5 }}>{p.detail}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div style={{
          ...
          <p style={{ color: "#e05a00", fontWeight: 700, fontSize: "0.82rem", textTransform: "uppercase", letterSpacing: ".06em", marginBottom: 10 }}>
            ⚠️ Side effects
          </p>
          <p style={{ color: "#555", fontSize: "0.938rem", lineHeight: 1.65 }}>{d.sideEffects}</p>
        </div>
        ...
        <div style={{
          ...
          <p style={{ color: d.color, fontWeight: 700, fontSize: "0.82rem", textTransform: "uppercase", letterSpacing: ".06em", marginBottom: 14 }}>
            🔍 Things to consider
          </p>
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {d.considerations.map((c, i) => (
              <div key={i} style={{
                ...
                <span style={{ color: d.color, fontWeight: 700, flexShrink: 0 }}>→</span>
                <span style={{ color: "#333", fontSize: "0.938rem", lineHeight: 1.5 }}>{c}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{
          ...
          {d.faqs.map((faq, i) => (
            <div key={i} style={{ marginBottom: i < d.faqs.length - 1 ? 10 : 0 }}>
              ...
              >
                <span style={{ color: openFaq === i ? "#fff" : d.color, fontWeight: 700, fontSize: "0.938rem" }}>
                  {faq.q}
                </span>
                <img
                  src="/img/arrow.webp"
              ...
              {openFaq === i && (
                <div style={{
                  ...
                  <p style={{ color: "#333", fontSize: "0.938rem", lineHeight: 1.6, margin: 0 }}>{faq.a}</p>
                </div>
              )}
            </div>
          ))}
      ...
      <div style={{
        ...
        <button onClick={onBack} style={{
          ...
          padding: "12px 24px", fontWeight: 700, fontSize: "0.875rem", cursor: "pointer",
        }}>
          ← Back
        </button>
          ...
          <div key={m.id} style={{ marginBottom: 12 }}>
            ...
            {open === m.id && (
              <div style={{ background: "#fff", border: "2px solid " + blue, borderTop: "none", borderRadius: "0 0 14px 14px", padding: "20px 24px" }}>
                <p style={{ color: blue, fontWeight: 700, fontSize: "0.938rem", marginBottom: 6 }}>{m.content.tagline}</p>
                <p style={{ color: "#555", fontSize: "0.875rem", marginBottom: 12, fontStyle: "italic" }}>{m.content.schedule}</p>
                <p style={{ fontWeight: 700, fontSize: "0.875rem", color: "#333", marginBottom: 8 }}>The process:</p>
                {m.content.process.map((p, i) => (
                  <div key={i} style={{ display: "flex", gap: 12, marginBottom: 8, padding: "8px 12px", background: "#f0f6ff", borderRadius: 8 }}>
                    <span style={{ color: blue, fontWeight: 700, minWidth: 100, fontSize: "0.875rem" }}>{p.time}</span>
                    <span style={{ color: "#333", fontSize: "0.875rem" }}>{p.detail}</span>
                  </div>
                ))}
                <p style={{ fontSize: "0.875rem", color: "#666", marginTop: 12, borderLeft: "3px solid " + blue, paddingLeft: 10 }}>{m.content.note}</p>
              </div>
            )}
          </div>
        ))}
    ...
    <div style={{ background: bg, minHeight: "100vh", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "32px 20px 120px", opacity: animating ? 0 : 1, transition: "opacity .2s" }}>
        <div style={{ maxWidth: 600, width: "100%" }}>
          ...
          <h2 style={{ textAlign: "center", color: "#fff", fontSize: "clamp(1.25rem, 3vw, 1.5rem)", fontWeight: 700, marginBottom: 28, lineHeight: 1.35 }}>{q.question}</h2>
            ...
            {q.options.map((opt, i) => {
              ...
              return (
                <div key={i} onClick={() => setSelected(opt)} style={{ background: isSelected ? blue : "#fff", border: "2px solid " + (isSelected ? blue : "rgba(255,255,255,0.8)"), borderRadius: 16, padding: "18px 24px", cursor: "pointer", display: "flex", alignItems: "center", gap: 16, transition: "all .2s", boxShadow: isSelected ? "0 4px 20px rgba(61,128,232,0.35)" : "0 2px 8px rgba(0,0,0,0.08)", transform: isSelected ? "scale(1.02)" : "scale(1)" }}>
                  ...
                  <span style={{ color: isSelected ? "#fff" : "#333", fontWeight: 600, fontSize: "0.938rem", lineHeight: 1.3 }}>{opt.label}</span>
                </div>
              );
            })}
          </div>
            ...
            <div key={r} style={{ background: "#fff", border: "3px solid " + res.color, borderRadius: 20, padding: "28px", marginBottom: 16, boxShadow: "0 4px 20px rgba(0,0,0,0.08)" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 16 }}>
                ...
                <div>
                  ...
                  <h3 style={{ color: res.color, fontWeight: 700, fontSize: "1.1rem", margin: 0 }}>{res.label}</h3>
                </div>
              </div>
              <p style={{ color: "#333", fontSize: "0.938rem", lineHeight: 1.7, marginBottom: 12 }}>{res.description}</p>
              <div style={{ background: res.color + "12", borderLeft: "4px solid " + res.color, borderRadius: "0 8px 8px 0", padding: "10px 14px", marginBottom: 16 }}>
                <p style={{ color: res.color, fontWeight: 600, fontSize: "0.938rem", margin: 0 }}>{res.suitable}</p>
              </div>
              {/* Learn more button */}
              <button
