...
export default function AdminDash() {
  ...
  async function handleFileUpload(e) {
    ...
    if (file.type !== "application/pdf") {
      showToast("Only PDF files are allowed.", true); return;
    }
    ...
    setForm(f => ({ ...f, url: publicUrl, storage_path: storagePath }));
    setUploading(false);
    setProgress(100);
  ...
  async function handleSave() {
    ...
    if (!form.url.trim())   { showToast("Please upload a file or enter a URL.", true); return; }
  ...
  return (
    <div style={s.bg}>
      ...
      <div style={s.main}>
        ...
        {/* Form */}
        <div style={s.card}>
          <h3 style={s.cardTitle}>{editing ? "Edit Resource" : "Add New Resource"}</h3>
          <div style={s.formGrid}>
            ...
            {/* File Upload */}
            <div style={{ gridColumn: "1/-1" }}>
              <label style={s.label}>Upload PDF from your computer</label>
              <div style={s.uploadBox}>
                <input
                  ...
                  id="fileInput"
                />
                <label htmlFor="fileInput" style={s.uploadBtn}>
                  {uploading ? `Uploading… ${progress}%` : "Choose PDF from your computer"}
                </label>
                ...
                {form.url && !uploading && (
                  <div style={s.urlPreview}>
                    <span style={{ color: "#3ecf8e" }}>File ready: </span>
                    <a href={form.url} target="_blank" rel="noopener noreferrer"
                       style={{ color: "#60a5fa", fontSize: "0.8rem", wordBreak: "break-all" }}>
                      {form.storage_path || form.url}
                    </a>
                  </div>
                )}

                <div style={{ marginTop: 10 }}>
                  <label style={{ ...s.label, fontSize: "0.72rem" }}>Or paste a URL manually</label>
                  <input
                    style={{ ...s.input, marginBottom: 0 }}
                    value={form.url}
                    onChange={e => setForm(f => ({ ...f, url: e.target.value, storage_path: "" }))}
                    placeholder="https://... or /pdfs/filename.pdf"
                  />
                </div>
              </div>
            </div>
          ...
          <div style={{ marginTop: 20, display: "flex", gap: 10 }}>
            <button onClick={handleSave} disabled={busy || uploading} style={s.btnSuccess}>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr style={{ background: "#1e2435" }}>
                  {["Title", "Tab", "Tags", "File", "Actions"].map(h => (
  ...
  <a href={r.url} target="_blank" rel="noopener noreferrer" style={{ color: "#60a5fa", fontSize: "0.75rem" }}>
    {r.storage_path ? "View File" : "Link"}
  </a>
</td>
                      <td style={s.td}>
...
const s = {
  ...
  uploadBox:   { background: "#0d1117", border: "1px dashed #2a3147", borderRadius: 8, padding: 16 },
  uploadBtn:   { display: "block", background: "#3D80E8", color: "#fff", borderRadius: 8, padding: "10px 18px", cursor: "pointer", fontWeight: 600, fontSize: "0.9rem", textAlign: "center", marginBottom: 10 },
  progressBar: { height: 6, background: "#252b3b", borderRadius: 99, overflow: "hidden", marginBottom: 10 },
  progressFill:{ height: "100%", background: "#3ecf8e", borderRadius: 99, transition: "width .3s" },
  urlPreview:  { fontSize: "0.82rem", marginBottom: 10, padding: "8px 12px", background: "rgba(62,207,142,.06)", borderRadius: 6, border: "1px solid rgba(62,207,142,.2)" },
